...
def send_telegram_order_notification(order):
    ...
    for item in order.items.all():
        code = getattr(item, 'product_code', '') or ''
        code_part = f" | کد: `{code}`" if code else ""
        items_text += f"▫️ {item.product_name}{code_part} | Qty: {item.quantity} | Price: {item.price}\n"
...
def product_list(request):
    ...
    if search:
        products = products.filter(Q(name__icontains=search) | Q(description__icontains=search) | Q(product_code__icontains=search))
    ...
    if request.method == 'POST':
        ...
        for item in cart.items.all():
            OrderItem.objects.create(
                ...
                product_code=getattr(item.product, 'product_code', '') or '',
